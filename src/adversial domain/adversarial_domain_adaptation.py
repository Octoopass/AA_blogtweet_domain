...
Unfortunately it is not suitable for the problem
"""
...
# Download NLTK data
for item in ['punkt', 'words', 'stopwords', 'averaged_perceptron_tagger']:
    try:
        nltk.data.find(f'tokenizers/{item}' if item == 'punkt' else 
                       f'corpora/{item}' if item in ['words', 'stopwords'] else f'taggers/{item}')
    except LookupError:
...
class Config:
    ...
    # Output files
    MODEL_FILE = 'dann_model.pth'
    RESULTS_FILE = 'dann_results.txt'
...
class UnifiedFeatureExtractor:
    """Unified feature extractor - same family as mixed_train.py."""
    ...
    def __init__(self, top_4grams=500, use_4grams=True):
    ...
    def extract_fixed_features(self, text):
        """Extract stylometric features - simplified version"""
        ...
        if n_words > 0:
            ...
            for i in range(1, 11):
                features[f'word_len_{i}_freq'] = sum(1 for l in word_lengths if l == i) / n_words
            features['word_len_10plus_freq'] = sum(1 for l in word_lengths if l >= 10) / n_words
        else:
        ...
        # Digit patterns
        features['digit_freq'] = sum(1 for c in text if c.isdigit()) / total_chars if total_chars else 0
        features['two_digit_freq'] = len(re.findall(r'\d{2}', text)) / total_chars if total_chars else 0
        features['three_digit_freq'] = len(re.findall(r'\d{3}', text)) / total_chars if total_chars else 0
        
        # Function words
        if n_words > 0:
            features['function_words_freq'] = sum(1 for w in words_alpha if w.lower() in self.FUNCTION_WORDS) / n_words
        else:
            features['function_words_freq'] = 0
        ...
        try:
            ...
            for p in self.COMMON_POS:
                features[f'pos_{p}_freq'] = pos_counts.get(p, 0) / len(pos_tags) if pos_tags else 0
        except:
            for p in self.COMMON_POS:
                features[f'pos_{p}_freq'] = 0
        
        # Misspellings
        if n_words > 0:
            misspelled = sum(1 for w in words_alpha if w.lower() not in self.english_words and w.lower() not in self.stop_words)
            features['misspelling_freq'] = misspelled / n_words
        else:
    ...
    def build_4gram_vocabulary(self, blog_texts, tweet_texts):
        """Build character 4-gram vocabulary from both domains"""
        print("  Building 4-gram vocabulary...")
        all_texts = blog_texts + tweet_texts
        ...
        for text in all_texts:
            if pd.notna(text) and str(text).strip():
                ...
                for i in range(len(text_clean) - 3):
                    ...
                    if all(c.isalpha() or c.isspace() for c in ngram):
                        ngram_counts[ngram] += 1
        
        self.vocab_4grams = [ng for ng, _ in ngram_counts.most_common(self.top_4grams)]
        self.feature_names = self._build_fixed_feature_names()
        print(f"  Selected top {len(self.vocab_4grams)} 4-grams")
    ...
    def extract_4gram_features(self, text):
        """Extract 4-gram features"""
        if not self.vocab_4grams:
            return {}
        ...
        for i in range(len(text_clean) - 3):
            ngram = text_clean[i:i+4]
            if ngram in self.vocab_4grams:
                ngram_counts[ngram] += 1
        
        total_ngrams = sum(ngram_counts.values()) or 1
        return {f'4gram_{ng}': ngram_counts.get(ng, 0) / total_ngrams for ng in self.vocab_4grams}
    
    def process_texts(self, texts, labels, dataset_name):
        """Process texts and extract all features"""
        ...
        for text, label in zip(texts, labels):
            fixed_feats = self.extract_fixed_features(text)
            if fixed_feats is None:
                continue
            
            if self.use_4grams and self.vocab_4grams:
                ngram_feats = self.extract_4gram_features(text)
                fixed_feats.update(ngram_feats)
            
            features_list.append(fixed_feats)
...
def plot_training_history(history):
    ...
    plt.tight_layout()
    plt.savefig('dann_training_history.png', dpi=300, bbox_inches='tight')
    print("\n[PLOT] Training history saved to dann_training_history.png")
